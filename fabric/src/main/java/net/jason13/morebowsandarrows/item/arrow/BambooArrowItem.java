...
public class BambooArrowItem extends ArrowItem {
    ...
    public void appendHoverText(@NotNull ItemStack itemStack, @Nullable Level level, List<Component> tooltip, @NotNull TooltipFlag tooltipFlag) {
        tooltip.add(Component.translatable("morebowsandarrows.text.bamboo_arrow_lore"));
        tooltip.add(Component.translatable("morebowsandarrows.text.bamboo_arrow_damage"));
    ...
    public @NotNull AbstractArrow createArrow(@NotNull Level level, @NotNull ItemStack arrowItem, @NotNull LivingEntity liveEntity) {
        if (true) {
            return new BambooArrowEntity(level, liveEntity);
        } else {
